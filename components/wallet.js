        ...
        <div className={styles.wallet}>
            {(() => {
                if (wallet.error?.name) {
                    return (
                        <button className={cx(styles.button, { onto: !!ontoIcon }, { tp: !!tpIcon })} onClick={wallet.reset()}>
                            {t("connect-retry")}
                        </button>
                    )
                }
                ...
                if (wallet.status === "connected") {
                    return (
                        <button className={cx(styles.button, styles.active, { onto: !!ontoIcon }, { tp: !!tpIcon })} onClick={() => wallet.reset()}>
            ...
            {wallet.account && (
                <Link href="/backpack">
                    <span className={styles.backpack}></span>
                </Link>
            )}
            
            {wallet.account && <span className={styles.account}>{formatAddress(wallet.account)}</span>}

            {wallet.account && <span className={styles.balance}> {wallet.balance === "-1" ? "..." : `${parseFloat(ethersUtils.formatEther(wallet.balance)).toFixed(2)} BNB`} </span>}
